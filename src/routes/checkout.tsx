...
function CheckoutPage() {
  ...
  const submitOrder = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.error("[Prihika Checkout] Submit order failed", submitError);
    ...
    return (
      <div className="container mx-auto px-5 py-24 text-center max-w-md">
        ...
        <p className="mt-4 text-muted-foreground">
          Thank you. A confirmation has been sent to your email. Your pieces will arrive in 3-5
          days.
        </p>
        <Link
          to="/"
          className="mt-8 inline-block px-7 py-3 rounded-full"
            ...
            <div className="space-y-3">
              {["UPI", "Credit / Debit Card", "Net Banking", "Cash on Delivery"].map(
                (method, index) => (
                  <label
                    key={method}
                    className="flex items-center gap-3 p-4 rounded-2xl border border-border cursor-pointer hover:border-rose-gold"
                  >
                    <input
                      type="radio"
                      name="pay"
        ...
        <aside className="bg-secondary rounded-3xl p-8 h-fit sticky top-24 space-y-5">
          ...
          {cart.map((item) => (
            <div key={item.id} className="flex gap-3">
              {item.image ? (
                <img src={item.image} className="h-16 w-14 object-cover rounded-lg" alt="" />
              ) : null}
          ...
          <div className="border-t border-border pt-4 space-y-2 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span>{subtotal > 5000 || subtotal === 0 ? "Free" : formatPrice(200)}</span>
            </div>
  ...
  return (
    <div>
      ...
      <input
        ...
        type={type}
        required
        className="w-full bg-card border border-border rounded-xl px-4 py-3 outline-none focus:border-rose-gold transition-colors"
      />
    </div>
  );
}
