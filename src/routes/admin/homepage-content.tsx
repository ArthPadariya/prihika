...
export const Route = createFileRoute("/admin/homepage-content")({
  head: () => ({
    meta: [
      { title: "Homepage Content - Prihika Admin" },
...
function HomepageContentPage() {
  ...
  const submit = async () => {
    ...
    try {
      ...
      console.error("[Prihika CMS] Homepage content save failed", saveError);
      toast.error(
        saveError instanceof Error ? saveError.message : "Homepage content could not be saved.",
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <AdminLayout
      ...
      subtitle="Edit hero copy, CTA labels, hero imagery, badge text, and live storefront preview."
    >
      {loading ? (
        ...
        <div className="grid gap-6 xl:grid-cols-[1fr_.85fr]">
          <section className="rounded-lg border border-white/10 bg-white/[0.045] p-5">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field
                ...
                onChange={(value) => setForm((current) => ({ ...current, hero_badge: value }))}
              />
              <Field
                label="Hero Title"
                value={form.hero_title}
                onChange={(value) => setForm((current) => ({ ...current, hero_title: value }))}
              />
              <Field
                label="Primary CTA"
                value={form.cta_primary}
                onChange={(value) => setForm((current) => ({ ...current, cta_primary: value }))}
              />
              <Field
                label="Primary Link"
                value={form.cta_primary_link}
                onChange={(value) =>
                  setForm((current) => ({ ...current, cta_primary_link: value }))
                }
              />
              <Field
                label="Secondary CTA"
                value={form.cta_secondary}
                onChange={(value) => setForm((current) => ({ ...current, cta_secondary: value }))}
              />
              <Field
                label="Secondary Link"
                value={form.cta_secondary_link}
                onChange={(value) =>
                  setForm((current) => ({ ...current, cta_secondary_link: value }))
                }
              />
            </div>
            <textarea
              ...
              onChange={(event) =>
                setForm((current) => ({ ...current, hero_subtitle: event.target.value }))
              }
              rows={4}
              placeholder="Hero subtitle"
              className="mt-4 w-full rounded-lg border border-white/10 bg-black/25 px-3 py-3 text-sm text-white outline-none placeholder:text-[#f6ead0]/35"
            />
            <textarea
              value={form.hero_featured_note}
              onChange={(event) =>
                setForm((current) => ({ ...current, hero_featured_note: event.target.value }))
              }
              rows={2}
              placeholder="Featured note below hero CTAs"
              className="mt-4 w-full rounded-lg border border-white/10 bg-black/25 px-3 py-3 text-sm text-white outline-none placeholder:text-[#f6ead0]/35"
            />
            <textarea
              value={form.testimonial_text}
              onChange={(event) =>
                setForm((current) => ({ ...current, testimonial_text: event.target.value }))
              }
              rows={2}
              placeholder="Optional homepage testimonial text"
              className="mt-4 w-full rounded-lg border border-white/10 bg-black/25 px-3 py-3 text-sm text-white outline-none placeholder:text-[#f6ead0]/35"
            />
            <div className="mt-5">
              <UploadDropzone
                ...
                multiple={false}
                onUploaded={(urls) =>
                  setForm((current) => ({ ...current, hero_image: urls[0] ?? current.hero_image }))
                }
              />
            </div>
            ...
            <button
              disabled={saving}
              onClick={() => void submit()}
              className="mt-5 rounded-lg bg-[#d7b46a] px-5 py-3 text-sm font-semibold text-black hover:bg-[#f4d58d] disabled:opacity-60"
            >
          ...
          <section className="rounded-lg border border-white/10 bg-white/[0.045] p-5">
            <p className="mb-4 text-xs uppercase tracking-[0.22em] text-[#d7b46a]/80">
              Live Preview
            </p>
            <div className="overflow-hidden rounded-2xl bg-[#f9f4ea] text-[#241b17]">
              <div className="grid gap-5 p-6 md:grid-cols-2 md:items-center">
                <div>
                  {form.hero_badge ? (
                    <p className="inline-flex items-center gap-2 text-[10px] uppercase tracking-[0.28em] text-[#a05b52]">
                      <Sparkles className="h-3 w-3" /> {form.hero_badge}
                    </p>
                  ) : null}
                  ...
                  <p className="mt-4 text-sm leading-relaxed text-[#6f625d]">
                    {form.hero_subtitle || "Hero subtitle preview"}
                  </p>
                  <div className="mt-6 flex flex-wrap gap-2">
                    {form.cta_primary ? (
                      <span className="rounded-full bg-[#a05b52] px-4 py-2 text-xs uppercase tracking-widest text-white">
                        {form.cta_primary}
                      </span>
                    ) : null}
                    {form.cta_secondary ? (
                      <span className="rounded-full border border-[#241b17]/25 px-4 py-2 text-xs uppercase tracking-widest">
                        {form.cta_secondary}
                      </span>
                    ) : null}
                  </div>
