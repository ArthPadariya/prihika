...
export const Route = createFileRoute("/admin/featured-sections")({
  head: () => ({
    meta: [
      { title: "Featured Sections - Prihika Admin" },
...
function FeaturedSectionsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(
        loadError instanceof Error ? loadError.message : "Featured sections could not load.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const submit = async () => {
    try {
      ...
      console.error("[Prihika CMS] Featured section save failed", submitError);
      toast.error(
        submitError instanceof Error ? submitError.message : "Featured section could not be saved.",
      );
    }
  };
  ...
  const move = async (section: FeaturedSection, direction: -1 | 1) => {
    ...
    try {
      await Promise.all([
        saveFeaturedSection(
          { ...section, display_order: next.display_order ?? index + direction },
          section.id,
        ),
      ...
      console.error("[Prihika CMS] Featured section reorder failed", moveError);
      toast.error(
        moveError instanceof Error
          ? moveError.message
          : "Featured section order could not be saved.",
      );
    }
  };

  const toggle = async (section: FeaturedSection) => {
    try {
      await saveFeaturedSection({ ...section, active: !section.active }, section.id);
      toast.success(section.active ? "Featured section hidden." : "Featured section activated.");
      await load();
    } catch (toggleError) {
      console.error("[Prihika CMS] Featured section toggle failed", toggleError);
      toast.error(
        toggleError instanceof Error
          ? toggleError.message
          : "Featured section could not be updated.",
      );
    }
  };

  const remove = async (section: FeaturedSection) => {
    try {
      await deleteFeaturedSection(section.id);
      toast.success("Featured section deleted.");
      await load();
    } catch (deleteError) {
      console.error("[Prihika CMS] Featured section delete failed", deleteError);
      toast.error(
        deleteError instanceof Error
          ? deleteError.message
          : "Featured section could not be deleted.",
      );
    }
  };
  ...
  return (
    <AdminLayout
      ...
      subtitle="Assign CMS products to homepage bestsellers, trending grids, category headings, and collection headings."
    >
      ...
      {loading ? (
          ...
          {sections.map((section, index) => (
            ...
            >
              <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div>
                  <p className="text-xs uppercase tracking-[0.22em] text-[#d7b46a]/75">
                    {section.section_key}
                  </p>
                  ...
                  <p className="mt-3 text-xs text-[#f6ead0]/45">
                    {(section.product_ids ?? [])
                      ...
                      .join(", ") || "No products assigned"}
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <span className="rounded-full border border-[#d7b46a]/30 px-2.5 py-2 text-xs text-[#f4d58d]">
                    {section.active ? "Active" : "Inactive"} - order{" "}
        ...
        <EmptyState
          ...
          description="Create sections to control homepage product grids and headings."
        />
      )}
      ...
      <Modal
        ...
        description="Use section keys such as categories, bestsellers, collections, trending, testimonials, or instagram."
        wide
      >
        <div className="grid gap-5">
          <div className="grid gap-4 md:grid-cols-2">
            <Field
              ...
              onChange={(value) => setForm((current) => ({ ...current, section_key: value }))}
            />
            <Field
              label="Eyebrow"
              value={form.eyebrow}
          ...
          <div className="rounded-lg border border-white/10 bg-white/[0.035] p-3">
            <p className="mb-3 text-xs uppercase tracking-[0.18em] text-[#d7b46a]/75">
              Assigned Products
            </p>
            <div className="grid max-h-72 gap-2 overflow-y-auto sm:grid-cols-2">
              {products.map((product) => (
                <label
                  key={product.id}
                  className="flex items-center gap-3 rounded-lg border border-white/10 bg-black/20 p-3 text-sm text-[#f6ead0]/75"
                >
                  <input
                    type="checkbox"
