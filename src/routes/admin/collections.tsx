...
export const Route = createFileRoute("/admin/collections")({
  head: () => ({
    meta: [
      { title: "Collections CMS - Prihika Admin" },
...
function CollectionsAdminPage() {
  ...
  const submit = async () => {
    try {
      ...
      console.error("[Prihika CMS] Collection save failed", submitError);
      toast.error(
        submitError instanceof Error ? submitError.message : "Collection could not be saved.",
      );
    }
  };
  ...
  const move = async (collection: Collection, direction: -1 | 1) => {
    ...
    try {
      await Promise.all([
        saveCollection(
          { ...collection, display_order: next.display_order ?? index + direction },
          collection.id,
        ),
      ...
      console.error("[Prihika CMS] Collection reorder failed", moveError);
      toast.error(
        moveError instanceof Error ? moveError.message : "Collection order could not be saved.",
      );
    }
  };
  ...
  const toggle = async (collection: Collection) => {
    try {
      await saveCollection({ ...collection, featured: !collection.featured }, collection.id);
      toast.success(collection.featured ? "Collection hidden." : "Collection activated.");
      await load();
    } catch (toggleError) {
      console.error("[Prihika CMS] Collection toggle failed", toggleError);
      toast.error(
        toggleError instanceof Error ? toggleError.message : "Collection could not be updated.",
      );
    }
  };
  ...
  const remove = async (collection: Collection) => {
    try {
      ...
      console.error("[Prihika CMS] Collection delete failed", deleteError);
      toast.error(
        deleteError instanceof Error ? deleteError.message : "Collection could not be deleted.",
      );
    }
  };
  ...
  return (
    <AdminLayout
      title="Collections"
      subtitle="Manage storefront collection stories, cover images, SEO text, ordering, and active state."
    >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {items.map((collection, index) => (
            <div
              key={collection.id}
              className="grid gap-4 rounded-lg border border-white/10 bg-white/[0.045] p-4 lg:grid-cols-[220px_1fr]"
            >
              ...
              <div className="flex flex-col justify-between gap-4">
                <div>
                  ...
                  <p className="mt-4 max-w-2xl text-sm text-[#f6ead0]/60">
                    {collection.description}
                  </p>
                  <span className="mt-4 inline-flex rounded-full border border-[#d7b46a]/30 px-2.5 py-1 text-xs text-[#f4d58d]">
                    {collection.featured ? "Active" : "Inactive"} - order{" "}
                    {collection.display_order ?? index}
                  </span>
                </div>
        ...
        <EmptyState
          ...
          description="Create collections to power the homepage and collections page."
        />
      )}
      ...
      <Modal
        ...
        description="Upload a cover image and add collection metadata."
        wide
      >
        <div className="grid gap-5">
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field
              ...
              onChange={(value) => setForm((current) => ({ ...current, seo_title: value }))}
            />
            <Field
              label="Display Order"
              type="number"
          ...
          <UploadDropzone
            ...
            multiple={false}
            onUploaded={(urls) =>
              setForm((current) => ({ ...current, cover_image: urls[0] ?? current.cover_image }))
            }
          />
          {form.cover_image ? (
            <img src={form.cover_image} alt="" className="h-32 w-24 rounded-lg object-cover" />
          ) : null}
