...
export const Route = createFileRoute("/admin/categories")({
  head: () => ({ meta: [{ title: "Categories CMS - Prihika Admin" }, { name: "robots", content: "noindex,nofollow" }] }),
...
function CategoriesPage() {
  ...
  const move = async (category: Category, direction: -1 | 1) => {
    ...
    if (!next) return;
    await Promise.all([
      saveCategory({ ...category, display_order: next.display_order ?? index + direction }, category.id),
  ...
  return (
    <AdminLayout title="Categories" subtitle="Control homepage category cards, images, ordering, descriptions, and active state.">
      ...
      {loading ? <AdminCardSkeleton count={4} /> : items.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {items.map((category, index) => (
            <div key={category.id} className="overflow-hidden rounded-lg border border-white/10 bg-white/[0.045]">
              ...
              <div className="space-y-3 p-4">
                ...
                <span className="inline-flex rounded-full border border-[#d7b46a]/30 px-2.5 py-1 text-xs text-[#f4d58d]">
                  {category.featured ? "Active" : "Inactive"} - order {category.display_order ?? index}
                </span>
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => openForm(category)} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-white hover:bg-white/10">Edit</button>
                  <button onClick={() => void move(category, -1)} className="rounded-lg border border-white/10 p-2 text-[#f6ead0]/65 hover:bg-white/10"><GripVertical className="h-4 w-4" /></button>
                  <button onClick={() => void saveCategory({ ...category, featured: !category.featured }, category.id).then(load)} className="rounded-lg border border-white/10 px-3 py-2 text-sm text-[#f4d58d] hover:bg-[#d7b46a]/10">Toggle</button>
                  <button onClick={() => void deleteCategory(category.id).then(load)} className="rounded-lg border border-red-300/20 p-2 text-red-300 hover:bg-red-500/10"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </div>
          ))}
        </div>
      ) : <EmptyState title="No categories yet" description="Add categories to power homepage category cards and shop filtering." />}

      <Modal open={open} onOpenChange={setOpen} title={editing ? "Edit Category" : "Add Category"} description="Upload category imagery and set display order." wide>
        <div className="grid gap-5">
          ...
          <UploadDropzone bucket="categories" folder="category-cards" multiple={false} onUploaded={(urls) => setForm((current) => ({ ...current, image: urls[0] ?? current.image }))} />
          {form.image ? <img src={form.image} alt="" className="h-28 w-24 rounded-lg object-cover" /> : null}
          ...
          <button onClick={() => void submit()} className="rounded-lg bg-[#d7b46a] px-4 py-3 text-sm font-semibold text-black hover:bg-[#f4d58d]">Save Category</button>
        </div>
      </Modal>
    </AdminLayout>
  );
