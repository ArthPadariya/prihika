...
function WishlistPage() {
  ...
  return (
    <div className="container mx-auto px-5 lg:px-10 py-12">
      ...
      <p className="text-muted-foreground mb-12">{items.length} saved {items.length === 1 ? "piece" : "pieces"}</p>
      {items.length === 0 ? (
        <div className="text-center py-20">
          <p className="font-display text-2xl">Nothing saved yet</p>
          <Link to="/shop" className="mt-4 inline-block text-rose-gold underline">Discover the shop</Link>
        </div>
      ) : (
